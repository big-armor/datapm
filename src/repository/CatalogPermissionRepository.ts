...
export async function grantUserCatalogPermission({
  ...
  relations?: string[]
      ...
      // Check that the user does not already have this permission. 
        ...
        const savedEntry =  await transaction.save(entry)
          .catch(error => {console.log(`error saving UserCatalogPermission ${error}`)});

        console.log(`returning ${savedEntry}`);

        return savedEntry;
      });

    console.log(`user catalog permission ${userCatalogPermission}`);
...
async function getUserCatalogPermission({
  ...
  export class UserCatalogPermissionRepository extends Repository<UserCatalogPermission> {
    ...
    async findByUser({
      ...
      return this.manager.getRepository(UserCatalogPermission)
      .createQueryBuilder(ALIAS)
      .where({ userId: user.id })
      .addRelations(ALIAS, relations).getMany()
