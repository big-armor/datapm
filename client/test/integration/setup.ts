...
export let registryServerPort: number = Math.floor(Math.random() * (65535 - 1024) + 1024);
export let dataServerPort: number = Math.floor(Math.random() * (65535 - 1024) + 1024);

before(async function () {
	const npmPackageFileContents = fs.readFileSync("package.json");
	const packageFile = JSON.parse(npmPackageFileContents.toString());
	const registryServerVersion: string = packageFile.dependencies["datapm-lib"].replace(/[^0-9.]/g, "");
...
after(async function () {
	...
	try {
		let pids = pidtree(registryServerProcess.pid, { root: true });
	...
	try {
		let pids = pidtree(testDataServerProcess.pid, { root: true });
