...
const s3SinkPrompts = ["Do you want to use the default options?", "File Format?", "Region?", "S3 Bucket?", "S3 Path?"];
...
describe("S3 Sink Test", function () {
    ...
    it("Should import data without error", async function () {
        const prompts = getS3SinkPromptInputs([KEYS.DOWN, "", region, "", s3OutputPath]);
