...
function copyAppManfifest(directory) {
	...
	var xmlBuffer = fs.readFileSync("./appxmanifest.xml");
	var xmlString = xmlBuffer.toString();

	// Use cheerio to parse the xml and extract the version number
	var manifestJson = convert.xml2json(xmlString, { compact: true, spaces: 4 });
	var manifestObject = JSON.parse(manifestJson);
	var versionNumber = manifestObject.Package.Identity._attributes.Version;

	var versionParts = versionNumber.split(".");
	var lastNumber = Number.parseInt(versionParts[3]) + 1;
