...
import { PackageFile, PublishMethod, RegistryReference } from "datapm-lib";
...
import { ParameterOption, ParameterType } from "../util/parameters/Parameter";
...
export class PublishJob extends Job<PublishJobResult> {
    ...
    async _execute(): Promise<JobResult<PublishJobResult>> {
        ...
        try {
            packageFileWithContext = await getPackage(this.args.reference, "canonicalIfAvailable");
        } catch (error) {
