...
import { CLIJobContext } from "../command/CommandTaskUtil";
...
import { ParameterAnswer, ParameterType } from "../util/parameters/Parameter";
...
export class PackageJob extends Job<PackageJobResult> {
    constructor(private jobContext: CLIJobContext, private args: PackageJobArguments) {
        super(jobContext);
    }
    ...
    async _execute(): Promise<JobResult<PackageJobResult>> {
        ...
        if (sourceDescription == null) {
            this.jobContext.print(
                "ERROR",
                "No source impelementation found for " + maybeRepositoryDescription.getType()
            );
            return { exitCode: 1 };
        }
        ...
        const sourceInspectionContext: SourceInspectionContext = {
            ...
            parameterPrompt: async (parameters) => {
                await this.jobContext.parameterPrompt(parameters);
            }
        };
