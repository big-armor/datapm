...
import { inspectStreamSet, inspectSource } from "./PackageCommand";
...
export async function updatePackage(argv: UpdateArguments): Promise<void> {
	...
	for (const sourceObject of oldPackageFile.sources) {
		...
		for (const streamSet of uriInspectionResults.streamSetPreviews) {
			...
			streamSets.push({
				...
				schemaTitles: streamInspectionResult.schemas.map((s) => s.title as string),
				slug: streamSet.slug,
	...
	await publishCommand.handleCommand({ reference: packageFileLocation, defaults: argv.defaults });
