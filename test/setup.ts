...
import { Catalog, Maybe, MyCatalogsQuery, User } from './registry-client'
...
before(async ()=> {
  ...
  environment = await new DockerComposeEnvironment(composeFilePath, composeFile)
    .withWaitStrategy("postgres",Wait.forLogMessage("listening on IPv4 address \"0.0.0.0\", port 5432"))
    .withWaitStrategy("datapm-registry",Wait.forLogMessage("🚀 Server ready at http://localhost:4000"))
    .up();
...
async function createUser(firstName:string,lastName:string,username:string,emailAddress:string,password:string):Promise<ApolloClient<NormalizedCacheObject>> {
  ...
  return await new Promise((resolve,reject) => {
    ...
    .then(result =>  {
      expect(result != null).equal(true);
      expect(result!["data"].createMe != null).equal(true);
      expect(createFailed,"User create should not fail").equal(false);
...
describe("Authentication Tests", async () => {
  ...
  it("login for incorrect user should fail", async () => {
    ...
    .catch(error => {
      console.log("Succes: Login failed when it should")
      loginFailed = true;
    })
    .then(result =>  {
      expect(loginFailed,"No user should be returned, because they are not loggedin").equal(true);
    });
  });
  ...
  it("Get catalogs for user A & B", async function() {
    ...
    return userAClient.query<MyCatalogsQuery>({
      query: gql`
      query MyCatalogs {
        myCatalogs {
          identifier {
