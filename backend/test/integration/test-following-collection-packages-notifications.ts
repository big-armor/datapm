...
describe("Follow Collection's Packages Notifications Tests", async () => {
    ...
    it("Should send email after daily notification updates not containing package because of the following options selected", async () => {
        ...
        expect(userBEmail.text).to.not.contain("follow-col-packages-user-a published version 1.0.1");
        expect(userBEmail.text).to.contain("http://localhost:4200/follow-col-packages-user-b#user-following");
    });
    ...
    it("Should send email after daily notification updates containing package because of major change", async () => {
        ...
        expect(userBEmail.text).to.contain("added package user-a-col-packages-2/follow-test");
        expect(userBEmail.text).to.contain("follow-col-packages-user-a published version 2.0.0");
        expect(userBEmail.text).to.contain("http://localhost:4200/follow-col-packages-user-b#user-following");
    });
});
