...
export class ImageStorageService {
    ...
    public async saveImage(itemId: number,
                           image: FileUpload,
                           imageType: ImageType,
                           context: AuthenticatedContext): Promise<Image> {
    ...
    public async createImage(itemId: number,
                             image: FileUpload,
                             imageType: ImageType,
                             context: AuthenticatedContext): Promise<Image> {
        const fileName = uuid();
    ...
    public async updateImage(imageEntity: Image,
                             image: FileUpload,
                             imageType: ImageType,
                             context: AuthenticatedContext): Promise<Image> {
        const fileStream = image.createReadStream();
    ...
    public async findImage(itemId: number,
                           imageType: ImageType,
                           context: AuthenticatedContext): Promise<Image | undefined> {
        const repository = this.getRepository(context);
        return repository.findOneEntityReferenceIdAndType(itemId, imageType);
    }

    private buildImageEntity(imageId: string, itemId: number, userId: number, imageType: ImageType, mimeType: string): Image {
