...
class NodeJSEncryptionEngine implements EncryptionEngine{
    ...
    encryptValue(value: string): string {
        ...
        if(key == null)
            throw new Error("NODEJS_ENCRYPTION_KEY not set");

        const iv = crypto.randomBytes(16);

        const safeKey = crypto.createHash('sha256').update(String(key)).digest('base64').substr(0, 32);
        
        let cipher = crypto.createCipheriv('aes-256-cbc', Buffer.from(safeKey), iv);
    ...
    decryptValue(value: string): string {
        ...
        let iv = Buffer.from(jsonValue.iv, 'hex');
        let encryptedText = Buffer.from(jsonValue.encryptedData, 'hex');
        let decipher = crypto.createDecipheriv('aes-256-cbc', Buffer.from(safeKey), iv);
