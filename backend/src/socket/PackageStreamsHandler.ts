import { SocketResponseType, PackageStreamsRequest, PackageStreamsResponse, BatchRepositoryIdentifier, BatchesBySchema } from "datapm-lib";
...
export module SchemaInfoHandler {
    ...
    export async function handle(streamContext: SocketContext, packageStreamsRequest: PackageStreamsRequest): Promise<PackageStreamsResponse> {

        const packageEntity = await streamContext.connection.getCustomRepository(PackageRepository).findPackageOrFail({identifier: packageStreamsRequest.identifier});

        const dataBatches = await streamContext.connection.getCustomRepository(DataBatchRepository).findDefaultBatchesForPackage(packageEntity.id,packageStreamsRequest.identifier.majorVersion,["package","package.catalog"]);
