...
export const login = async (_0: any, { username, password }: { username: string, password: string }, context: AuthenticatedContext, info: any) => {
    const user = await context.connection.manager
        .getCustomRepository(UserRepository)
        .getUserByLogin(username, getGraphQlRelationName(info));

    if (user == null) {
        throw new ApolloError("User not found", AUTHENTICATION_ERROR.USER_NOT_FOUND);
    }

    const hash = hashPassword(password, user.passwordSalt);
    if (hash != user.passwordHash) {
        throw new ApolloError("Wrong credentials", AUTHENTICATION_ERROR.WRONG_CREDENTIALS);
    }
